{{ 
    config(
        materialized='table', 
        alias='STG_RYP_PICK_LOG_CFB',
	tags=["stg-ryp"]
    ) 
}}

WITH PICK_LOG_CFB AS (
    SELECT
        ...
        P.LEAGUE_TYPE_ID,
        P.ENTRY_ID,
        IFNULL( P.SHEET , P.ENTRY_ID ) AS SHEET_ID,
        P.PICK_PERIOD,
        'RYP' AS PRODUCT,
    ...
    FROM {{ source('RYP_PICK_LOGS', 'PICK_LOG_CFB') }} AS P
    INNER JOIN
