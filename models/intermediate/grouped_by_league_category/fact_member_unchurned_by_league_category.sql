{{ 
    config(
        materialized='table', 
        alias='FACT_MEMBER_UNCHURNED_BY_LEAGUE_CATEGORY',
	tags=["fact"]
    ) 
}}
...
PICK_DATES AS (
	...
	SOURCE,
        ...
        lag(PICK_DATE) OVER (PARTITION BY MEMBER_EMAIL, LEAGUE_CATEGORY ORDER BY PICK_DATE) AS PREVIOUS_PICK_DATE
...
SELECT * FROM FINAL
   ORDER BY MEMBER_EMAIL, UNCHURNED_DATE DESC